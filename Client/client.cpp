...
// client-side state (Board — как у тебя в game.h)
Board my_board;       // свои корабли (левая)
Board shots_board;    // куда стрелял (правая)
...
// Обработчик одиночного сообщения с сервера
...
void handle_server_message(const std::string& msg) {
    ...
    if (cmd == "YOUR_TURN") {
        ...
        // одиночные обновления (удобно держать для обратной совместимости)
        int r, c;
        if (cmd == "HIT" || cmd == "KILL" || cmd == "MISS") {
            iss >> r >> c;
            {
...
// блокирующее ожидание одного текстового ответа (используется в фазе placement)
...
void placeShipNetwork(SOCKET sock) {
    ...
    for (int length : shipLengths) {
        ...
        while (!isPlaced && running.load()) {
            ...
            prompt << "Разместите корабль длиной " << length << " (" << (placedCounts[length] + 1) << " из " << SHIP_LIMITS.at(length) << "): ";
            std::cout << prompt.str();
            ...
            case '\r': {
                ...
                // wait reply (blocking) — receiver thread ещё не запущ
...
// сетевой выстрел (использует condition_variable для результата)
void makeShotNetwork(SOCKET sock) {
    FirePlacement currentFire = { HEIGHT / 2, WIDTH / 2 };

    while (running.load()) {
        // ждём флага your_turn
        ...
        while (your_turn.load() && running.load()) {
            ...
            std::cout << "\nТвой ход! WASD - двигайся, ENTER - стрелять\n";
            wchar_t symbol = _getch();
            int maxRow = HEIGHT - 1;
            int maxCol = WIDTH - 1;
            ...
            case '\r': {
                ...
                // ждём ответ, который выдаст receiver_thread через condition_variable
                ...
                // если промах — завершаем ход (сервер должен прислать YOUR_TURN/OPPONENT_TURN контрольный пакет)
                if (result.rfind("MISS", 0) == 0 || result.rfind("ALREADY", 0) == 0) {
                    your_turn.store(false);
                    break;
                }
                // при HIT/KILL предполагаем, что сервер пришлёт YOUR_TURN снова для продолжения или не придёт -> rely на YOUR_TURN флаг
                break;
            }
            case L'Q': case 27:
                running.store(false);
...
int main(int argc, char* argv[]) {
    ...
    // Инициализация Board — делаем явную инициализацию по элементам (Board — фиксированная структура)
    {
        std::lock_guard<std::mutex> lg(boards_mtx);
    ...
    // размещаемся, в процессе placeShipNetwork использую blocking recv для ответов PLACED/INVALIDYOU
    placeShipNetwork(sock);

    // теперь игра стартует — запускаем приёмный поток
    std::thread recvThr(receiver_thread, sock);

    // запускаем сетевой режим выстрела (блокирует пока игра идёт)
