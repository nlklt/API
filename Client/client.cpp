...
#include "/API/ship_war/game.h"                // или путь "/API/ship_war/game.h" если у теб€ так
#include <winsock2.h>
#include <ws2tcpip.h>
...
void handle_server_message(const std::string& msg) {
    ...
    if (msg.rfind("BATCH\n", 0) == 0) {
        ...
        // ѕосле применени€ обновлений Ч перерисуем ƒќ—№ только ќƒ»Ќ раз.
    ...
    // —тара€ логика дл€ одиночных сообщений (HIT/MISS/KILL/INCOMING_* / YOUR_TURN / GAME_START / WIN / etc.)
